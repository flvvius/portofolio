    ...
    <Rise delay={delay}>
      <section id={shelf.slug} aria-labelledby={`shelf-${shelf.slug}`}>
        <div className="flex items-baseline justify-between gap-4 border-b border-ink/20 pb-2">
          <h2
            id={`shelf-${shelf.slug}`}
            className="font-mono text-[0.96rem] uppercase tracking-[0.18em] text-ink"
          >
            {shelf.label}
          </h2>
          <p className="font-mono text-[0.96rem] text-ink-soft">{shelf.aside}</p>
        </div>
          ...
          {shelf.entries.map((entry, index) => (
            ...
            >
              <div className="flex items-baseline justify-between gap-4">
                <h3 className="font-mono text-[1.38rem] leading-snug text-ink">
                  {entry.href ? (
                    <a
                      href={entry.href}
                      target="_blank"
                ...
                {entry.by && (
                  <span className="shrink-0 text-right font-mono text-[0.96rem] uppercase tracking-[0.12em] text-ink-soft">
                    {entry.by}
                  </span>
                )}
              </div>
            ...
            <Rise delay={120} className="col-span-12 lg:col-span-5">
              <div className="flex items-end justify-start gap-2 lg:justify-end">
                <ScribbleNote
                  ...
                  seed={37}
                  arrowClassName="w-10"
                  // Wide enough to hold "borrowed" on one line at this size:
                  // any narrower and the word overflows the box rather than
                  // wrapping, and lands on top of the bookshelf.
